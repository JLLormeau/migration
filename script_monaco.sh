...
for tenant in GCP INTRA AZR AWS
do 
  if [ -e $env/$apiUpper/$env"_"$tenant/$api/$api.yaml ]
  then
    test=` cat $env/$apiUpper/$env"_"$tenant/$api/$api.yaml| grep "^-" | grep -ve "- name" | wc -l`
    if [ "$test" -eq  0 ]
      then
        mv $env/$apiUpper/$env"_"$tenant/$api/$api.yaml $env/$apiUpper/$env"_"$tenant/$api/$api.yaml.skipped
      fi
  fi
...
then
 ...
 for tenant in GCP AWS AZR INTRA 
 do
  if [ $error -eq 0 ] 
  then 
    if [ -f $env/$apiUpper/$env"_"$tenant/$api/$api.yaml ]
    then
      mv $env/$apiUpper/$env"_"$tenant/$api/$api.yaml $env/$apiUpper/$env"_"$tenant/$api/$api.yaml.done
    fi
  else
    ...
    then
      ...
      then
        ...
        sed -i 's/^#ok\- name/\- name/g' $env/$apiUpper/$env"_"$tenant/$api/$api.yaml
