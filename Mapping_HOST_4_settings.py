...
LISTENTITY=[]
LISTENTITY=['HOST_GROUP', 'HOST', 'PROCESS_GROUP_INSTANCE']
#LISTENTITY=['HOST']
...
def mapping(DIC_MANAGED,DIC_SAAS):
    ...
    for id_managed in DIC_MANAGED:
        if id_managed not in DIC_MAPPING_OLD :
            for id_saas in DIC_SAAS:
                if DIC_SAAS[id_saas] == DIC_MANAGED[id_managed]:
                    #print(DIC_MANAGED[id_managed], DIC_SAAS[id_saas])
                    if id_managed == id_saas :
                        DIC_MAPPING_NEW[id_managed]=id_saas+";"+DIC_SAAS[id_saas]+";"+DIC_MANAGED[id_managed]                        
                        DIC_MAPPING_OLD[id_managed]=id_saas+";"+DIC_SAAS[id_saas]+";"+DIC_MANAGED[id_managed]
...
for api  in Mapping:
    ...
    if os.path.exists(DIRECTORY+'entity_'+api_name+'_id_source.csv'):
            #print(api, 'use local cache '+DIRECTORY+'entity_'+api_name+'_id_source.csv')
            file_test=True
    ...
    #previous mapping
    if os.path.exists(DIRECTORY+'entity_'+api_name+'_mapping.csv'):
            f= open(DIRECTORY+'entity_'+api_name+'_mapping.csv',encoding='utf-8')
            f = csv.reader(f,delimiter = ';')
            for row in f:
                if row != []:
                    if len(row)>2:
                        DIC_MAPPING_OLD[row[0]]=row[1]+";"+row[2]+";"+row[3]
    ...
    print(api, "total mapping=", len(DIC_MAPPING_OLD),"& new mapping=", len(DIC_MAPPING_NEW))
    	
    #generate mapping file
    fichier = open(DIRECTORY+'entity_'+api_name+'_mapping.csv', "w",encoding='utf-8')
    for id in DIC_MAPPING_OLD :
        fichier.write(id+";"+DIC_MAPPING_OLD[id]+"\n")
    fichier.close()

    #generate delta file
    fichier = open(DIRECTORY+'entity_'+api_name+'_new_mapping.csv', "w",encoding='utf-8')
    for id in DIC_MAPPING_NEW :
        fichier.write(id+";"+DIC_MAPPING_NEW[id]+"\n")
    fichier.close()
