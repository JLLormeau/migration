...
schemaIds={'KEY_REQUESTS':'builtin:settings.subscriptions.service','MUTED_REQUESTS':'builtin:settings.mutedrequests', 'HTTP_FAILURE':'builtin:failure-detection.service.http-parameters', 'GENERAL_FAILURE': 'builtin:failure-detection.service.general-parameters'}
        ...
        if error:
            ...
            if settings_object_json['items']: #if Object is not empty 
                for object in settings_object_json['items']: #Add each object to list
                    ...
                    #3-Get processGroupId, PGentityName and hostGroupId
                    hostGroupIds =[]

                    if entity_json['fromRelationships']:
                        processGroupId = entity_json['fromRelationships']['runsOn'][0]['id']
                        error,entity = get_entity(url, token, Api["ENTITY"], processGroupId)
                        entity_json = json.loads(entity)
                        PGentityName = entity_json['displayName']

                        if entity_json['fromRelationships']['runsOn'][0]["id"]:
                            list_hosts = entity_json['fromRelationships']['runsOn']
                            for h in list_hosts:
                                error,host = get_entity(url, token, Api["ENTITY"], h["id"])
                                host_json = json.loads(entity)
                                if "isInstanceOf" in host_json["fromRelationships"]:
                                    hostGroupIds.append(host_json["fromRelationships"]["isInstanceOf"]["id"])
                                else:
                                    continue
                        ...
                        object = {
                                ...
                                "PROCESS_GROUP_entityName": PGentityName,
                                "HOST_GROUP_ID": hostGroupIds,
                                "status":"to import"
                            }
                        
                        #5-Add result to list and create csv mapping
                        objects.append(object) 
                        print(entityName + ";" + serviceId + ";" + PGentityName  + ";" + processGroupId) 
                        ...
                        object = {
                                ...
                                "value" : value,
                                "entityName" : entityName,
                                "PROCESS_GROUP_entityName": "fromRelationships empty",
                                "HOST_GROUP_ID": "fromRelationships empty",
                                "status":"to import"
                            }
...
def main():
    ...
    print("\n//---Key requests--//")
    filename = "backup_service_key_requests-" + ENV.lower() + "-" + tenant + ".json" 
    backup_settings (ManagedTenant, ManagedToken, Api["SETTINGS_OBJECT"], schemaIds['KEY_REQUESTS'], filename)
    
    print("\n//---Muted requests--//")
    filename = "backup_service_muted_requests-" + ENV.lower() + "-" + tenant + ".json" 
    backup_settings (ManagedTenant, ManagedToken, Api["SETTINGS_OBJECT"], schemaIds['MUTED_REQUESTS'], filename)

    print("\n//---HTTP failure detection--//")
    filename = "backup_service_http_failure_detection-" + ENV.lower() + "-" + tenant + ".json" 
    backup_settings (ManagedTenant, ManagedToken, Api["SETTINGS_OBJECT"], schemaIds['HTTP_FAILURE'], filename)

    print("\n//---General failure detection--//")
    filename = "backup_service_general_failure_detection-" + ENV.lower() + "-" + tenant + ".json" 
    backup_settings (ManagedTenant, ManagedToken, Api["SETTINGS_OBJECT"], schemaIds['GENERAL_FAILURE'], filename)
